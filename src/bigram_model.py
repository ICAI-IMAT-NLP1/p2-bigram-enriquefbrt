...
def bigrams_count_to_probabilities(
    ...
    probabilities = torch.zeros(size=bigram_counts.shape)

    for chr1, char_bigrams in enumerate(bigram_counts):
        total_char = bigram_counts[chr1].sum()
        for chr2, _ in enumerate(char_bigrams):
            probabilities[chr1, chr2] = bigram_counts[chr1, chr2] / total_char
...
def calculate_neg_mean_log_likelihood(
    ...
    # TODO
    total_log_likelihood: torch.tensor = torch.Tensor()
    ...
    total_log_likelihood = torch.Tensor(
        [calculate_log_likelihood(
            ...
            char_to_index, start_token, 
            end_token)
         for word in words])

    # Calculate and return the negative mean log likelihood
    # TODO
    mean_log_likelihood: float = total_log_likelihood.mean()
...
def sample_next_character(
    ...
    # TODO
    next_char_index: int = int(torch.multinomial(current_probs, num_samples=1)[0])
...
def generate_name(
    ...
    while (current_char != end_token) and (len(generated_name) < max_length):
        ...
        current_char = generated_name[-1]

    return generated_name[:-1] if generated_name[-1] == end_token else generated_name
...
def calculate_log_likelihood(
    ...
    log_likelihood = torch.Tensor([
        bigram_probabilities[char_to_index[chr1.lower()], char_to_index[chr2.lower()]]
        for chr1, chr2 in zip(processed_word, processed_word[1:])
        ])

    return torch.log(log_likelihood).sum()
